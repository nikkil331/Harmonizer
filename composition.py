...
class Composition(object):
    ...
    def best_new_part(self, parts):
        second_parts = map(lambda x: self.create_part(x, 'data/barbershop/models/{0}_language_model_major.txt'.format(x),
                                            ['data/barbershop/models/{0}_{1}_translation_model_major_rhythm_threshold.txt'.format(y[0],x) for y in self._parts],
                                            ['data/barbershop/models/{0}_{1}_translation_model_major.txt'.format(y[0],x) for y in self._parts]), 
                                        parts)
...
def main():
    test_song = converter.parse('data/barbershop/classic_tags_0.xml')
    #test_song = corpus.parse('bach/bwv390')
    parts = ['0', '2', '3']
    c = Composition()
    c.add_part(1, test_song.parts[1].getElementsByClass('Measure')[1:])
    while parts:
        parts.remove(c.best_new_part(parts))
    c.save("barbershop.xml")
